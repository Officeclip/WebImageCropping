...
namespace CS.Web.UI
{
    ...
    public class CropImage : CompositeControl, INamingContainer
    {
        ...
        {
            get { return ViewState["IPP"] == null ? string.Empty : Convert.ToString(ViewState["IPP"]); }
            set { ViewState["IPP"] = value; }
        }
        ...
        private bool GetRawSize(string rawimage)
        {
            ...
            catch (Exception ex)
            {
                ...
                catch (Exception ex2)
                {
                    ////try
                    ////{
                    ////    if (this.ImagePath.ToLower().Contains("binaryimage.aspx"))
                    ////    {
                    ////        string imagePhysicalPath =
                    ////            this.ImagePath.Substring(
                    ////                this.ImagePath.IndexOf("id=", StringComparison.InvariantCulture) + 3);
                    ////        imagePhysicalPath = FromBase64String(imagePhysicalPath);
                    ////        image = drawing.Image.FromFile(imagePhysicalPath);
